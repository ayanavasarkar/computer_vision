...
if __name__ == '__main__':
    ...
    image_size = (70 + half_w * 2, 70 + half_w * 2)

    img = mpimg.imread(filename)

    # rgb case, transform to grayscale
    if len(img.shape) > 2:
        img = (0.2126 * img[:, :, 0] + 0.7152 * img[:, :, 1] + 0.0722 * img[:, :, 2]) / 255.0
    ...
    mpimg.imsave('3_15.png', target[half_w:-half_w, half_w:-half_w], cmap=plt.get_cmap('gray'))